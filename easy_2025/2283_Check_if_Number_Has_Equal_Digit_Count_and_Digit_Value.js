...
var digitCount = function(num) {
    ...
    // let digitCOunt = new Map();

    for(let i = 0; i < num.length; i ++){
        digitCount[+num[i]] ++; 
        // digitCount.set(num[i] ,(digitCount.get(num[i]) || 0 ) + 1)
    }

    for(let i = 0; i < num.length; i ++){
        if(digitCount[i] != +num[i]) return false; 
        // if((digitCount.get(i.toString()) || 0) != (parseInt(num[i]))) return false; 
    }
