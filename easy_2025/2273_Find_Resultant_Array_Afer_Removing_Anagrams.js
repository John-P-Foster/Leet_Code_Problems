...
var removeAnagrams = function(words) {
    const letterFreq = function(word){
    ...
    const areAnagrams = function(freq1, freq2){
    ...
    let prev =  letterFreq(words[0]); 
    for(let i = 1; i < words.length; i++){
        let freq = letterFreq(words[i]);
        if(!areAnagrams(prev, freq)){
            result.push(words[i]);
            prev = freq; 
        }
    }
...
const testCases = [
  ...
  for(let[words, expected] of testCases){
    const returned = removeAnagrams(words);
