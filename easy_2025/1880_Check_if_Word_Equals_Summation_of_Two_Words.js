...
// the letter values of each letter in s, which is
...
var isSumEqual = function(firstWord, secondWord, targetWord) {
    const getNumericalValue = function(string){
        let value = 0
        for(let char of string){
            value = (value * 10 ) + (char.charCodeAt(0) - 97)
        }
...
let testNumber = 1; 
for(let[firstWord, secondWord, targetWord, expected] of testCases){
    const returned = isSumEqual(firstWord, secondWord, targetWord); 
