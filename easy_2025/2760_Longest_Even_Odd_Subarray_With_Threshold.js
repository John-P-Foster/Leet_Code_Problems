...
// Hence, the answer is the length of the subarray, 3. We can show that 3 is the maximum possible achievable
//  length.
// Example 2:
...
var longestAlternatingSubarray = function(nums, threshold) {
    ...
    for(let i = 0; i < nums.length; i++){
        if(nums[i] <= threshold){
            if(nums[i] % 2 == evenOrOdd){
                current ++; 
                longest = Math.max(longest, current); 
                evenOrOdd = !evenOrOdd 
            }else{
                current = nums[i] % 2 === 0 ? 1 : 0;  
                evenOrOdd = nums[i] % 2 === 0 ? true : false;  
            }
        }else{
            current = 0; 
            evenOrOdd = false; 
...
let testNumber = 1; 
for(let[nums, threshold, expected] of testCases){
  const returned = longestAlternatingSubarray(nums, threshold);
