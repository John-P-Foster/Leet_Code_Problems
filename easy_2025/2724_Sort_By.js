...
var sortBy = function(arr, fn) {
    const transformed = arr.map(val => [fn(val), val]);
    transformed.sort((a,b) => a[0] - b[0]); 
    return transformed.map(val => val[1]) 
};

const testCases = [
    [[5, 4, 1, 2, 3], (x) => x, [1, 2, 3, 4, 5]],
    [[{"x": 1}, {"x": 0}, {"x": -1}], (d) => d.x, [{"x": -1}, {"x": 0}, {"x": 1}]],
    [ [[3, 4], [5, 2], [10, 1]], (x) => x[1], [[10, 1], [5, 2], [3, 4]]]
    
]


let testNumber = 1; 
for(let[arr, fn, expected] of testCases){
  const returned = sortBy(arr, fn);
