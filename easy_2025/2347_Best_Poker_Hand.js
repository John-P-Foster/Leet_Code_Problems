...
var bestHand = function(ranks, suits) {
    ...
    const ranksSeen = new Array(14).fill(0);
    const suitsSeen = new Array(4).fill(0); 

    for(const suit of suits){
        const suitIndex = suit.charCodeAt(0) - 'a'.charCodeAt(0);
        suitsSeen[suitIndex]++;
        if (suitsSeen[suitIndex] === 5) return "Flush";
    }

    let maxCountRank = 1;  
    for(const rank of ranks){
        ranksSeen[rank] ++
        maxCountRank = Math.max(ranksSeen[rank], maxCountRank)
    }

    if(maxCountRank >= 3) return "Three of a Kind"; 
    if(maxCountRank === 2) return "Pair";
...
let testNumber = 1; 
for(let[ranks, suits, expected] of testCases){
  const returned = bestHand(ranks, suits);
