...
// her candies very much, and she wants to eat the maximum number of different types of candies while still
...
var distributeCandies = function(candyType) {
    const targetCandies = candyType.length / 2; 
    let uniqueCandies = new Set(); 

    for(let i = 0; i < candyType.length ; i++){
        uniqueCandies.add(candyType[i]); 
        if(uniqueCandies.size === targetCandies) return targetCandies; 
    }
...
let testNumber = 1; 
for(let[candyType, expected] of testCases){
    const returned = distributeCandies(candyType);
