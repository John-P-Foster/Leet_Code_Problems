// You are given a 0-indexed circular string array words and a string target. A circular array means that
//  the array's end connects to the array's beginning.
...
var hamminDistance = function(words, target, startIndex) {
    ...
    for(let moves = 0;  moves < n; moves ++) {
        if (words[left] === target || words[right] === target) return moves;
        left = (left - 1 + n) % n;
        right = (right + 1) % n;
      }
...
const testCases = [
    [1,words = ["hello","i","am","leetcode","hello"], target = "hello", startIndex = 1],
    [1, words = ["a","b","leetcode"], target = "leetcode", startIndex = 0],
