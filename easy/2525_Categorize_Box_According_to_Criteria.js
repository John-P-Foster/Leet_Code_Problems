...
var categorizeBox = function(length, width, height, mass) {
    const maxDimension = 10 ** 4; 
    const maxVolume = 10 ** 9; 
    const maxMass = 100; 
    
    let bulky = false; 
    if((length >= maxDimension) ||  (height >= maxDimension) || (width >= maxDimension) || ((length * width * height) >= maxVolume)) bulky = true;
    
    let heavy = false; 
    if(mass >= maxMass) heavy = true; 
...
let testNumber = 1; 
for(let[length, width, height, mass, expected] of testCases){
    const returned = categorizeBox(length, width, height, mass);
