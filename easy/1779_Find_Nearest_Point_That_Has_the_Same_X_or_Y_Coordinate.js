...
//  represents that a point exists at (ai, bi). A point is valid if it shares the same x-coordinate 
...
var nearestValidPoint = function(x, y, points) {
    ...
    for(let i = 0; i < points.length; i  ++){
        if(points[i][0] === x || points[i][1] === y){
            const manDistance = manhattanDistance([x,y], points[i]);
...
let testNumber  = 1; 
for(let [x, y, points, expected] of testCases){
    const returned = nearestValidPoint(x,y,points);
